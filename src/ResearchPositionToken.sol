...
contract ResearchPositionToken {
    ...
    function protocolTransfer(address from, address to, uint256 tokenId) external onlyMinter {
        if (to == address(0)) revert ZeroAddress();
        if (owners[tokenId] != from) revert TokenOwnerMismatch();
