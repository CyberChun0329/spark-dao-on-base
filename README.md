...
The current cost model uses calibration schema version 2. Its teaching rows are generated
by `TeachingGasCalibrationTest`; its research rows are generated by
`ResearchGasCalibration.t.sol`; `npm run simulate:teaching-cost` reads those CSVs and
writes the current files under `simulation_outputs/`. `npm run check:calibration` runs
the teaching and research generators, typecheck, reproducibility config check, simulation,
and a clean-worktree guard when the command is run inside a Git worktree. The old V1
pipeline is historical and should be preserved by a public `calibration-v1-final` tag
during the next public sync rather than remaining active in the live source. Do not
hand-fill gas or cost outputs.
