...
writes the V2 files under `simulation_outputs/`. `npm run check:calibration:v2` runs the
teaching and research V2 generators, typecheck, reproducibility config check, V2
simulation, and a clean-worktree guard when the command is run inside a Git worktree. The
older `simulate:teaching-cost` command and V1 output files remain in the repository only
as a historical reproducibility baseline. Do not hand-fill gas or cost outputs.
