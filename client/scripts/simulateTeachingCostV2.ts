...
const gasCsvPath = join(root, "teaching_gas_calibration_v2.csv");
const followupGasCsvPath = join(root, "teaching_followup_gas_calibration_v2.csv");
...
function validateTeachingRows(rows: TeachingGasRow[]) {
  if (rows.length !== 20) throw new Error(`Expected 20 teaching V2 rows, got ${rows.length}`);
  const seen = new Set<string>();
  for (const row of rows) {
    if (seen.has(row.path)) throw new Error(`Duplicate teaching V2 path: ${row.path}`);
    seen.add(row.path);
  ...
  for (const prefix of ["ORD", "FV", "CF", "TF"]) {
    for (const kind of ["NR", "ZS", "RB", "WM", "ML"]) {
      ...
      if (!seen.has(path)) throw new Error(`Missing teaching V2 path: ${path}`);
    }
  }
}
...
function validateFollowupRows(rows: FollowupGasRow[]) {
  if (rows.length !== 1) {
    throw new Error(`Expected exactly 1 teaching V2 follow-up primitive, got ${rows.length}`);
  }
...
const markdown = `# Teaching Cost Simulation V2

Generated from \`teaching_gas_calibration_v2.csv\`, \`teaching_followup_gas_calibration_v2.csv\`, and \`research_gas_calibration.csv\`.
...
writeFileSync(join(outDir, "teaching_cost_simulation_v2.csv"), simulationRows.join("\n"));
writeFileSync(
  join(outDir, "teaching_cost_simulation_v2.json"),
  JSON.stringify(
    {
      calibrationSchemaVersion,
      gasRows,
...
writeFileSync(join(outDir, "calibration_manifest_v2.json"), JSON.stringify(manifest, null, 2));
writeFileSync(join(outDir, "teaching_cost_simulation_v2.md"), markdown);

console.log(`Wrote ${join(outDir, "teaching_cost_simulation_v2.csv")}`);
console.log(`Wrote ${join(outDir, "teaching_cost_simulation_v2.json")}`);
console.log(`Wrote ${join(outDir, "calibration_manifest_v2.json")}`);
console.log(`Wrote ${join(outDir, "teaching_cost_simulation_v2.md")}`);
