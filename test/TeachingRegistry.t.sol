...
contract TeachingRegistryTest {
    ...
    function testCreateCourseTypeAndFreezeRoundOne() public {
        ...
        uint64 courseTypeId = registry.createTeachingCourseType("Linear Algebra", 1_000_000, 400_000, 0);
    ...
    function testTeachingAcknowledgeThenCounterpartyConfirmSettles() public {
        VM.prank(coordinator);
        uint64 courseTypeId = registry.createTeachingCourseType("Ack Geometry", 1_000_000, 400_000, 0);
    ...
    function testCustomerFirstAcknowledgeResearchBackedTeachingStillSnapshotsRewards() public {
        ...
        (, uint256[] memory amountsA) = registry.getTeachingRewardLedgerBuckets(assetId, layerOnePositionA);
        (, uint256[] memory amountsB) = registry.getTeachingRewardLedgerBuckets(assetId, layerOnePositionB);
    ...
    function testCoordinatorTeacherFaultRefundsCustomer() public {
        VM.prank(coordinator);
        ...
        VM.prank(coordinator);
        registry.coordinatorInvalidateTeacherFault(teachingNftId, 4);
    ...
    function testTransferredResearchPositionLetsNewHolderClaimTeachingReward() public {
        VM.startPrank(coordinator);
        uint64 assetId = registry.createResearchAsset("Transferable Research", "ipfs://transferable");
        ...
        assertTrue(researchToken.ownerOf(_researchTokenId(assetId, layerOnePosition)) == contributorTwo);
    ...
    function testWeightedMultiAssetTeachingDistribution() public {
        ...
        (, uint256[] memory amountsOne) = registry.getTeachingRewardLedgerBuckets(assetOne, positionOne);
        (, uint256[] memory amountsTwo) = registry.getTeachingRewardLedgerBuckets(assetTwo, positionTwo);
    ...
    function testMultiAssetMultiLayerDistributionConservesTotalValue() public {
        VM.startPrank(coordinator);
        uint64 assetOne = registry.createResearchAsset("Conservation One", "ipfs://conservation-one");
        ...
        uint64 assetTwo = registry.createResearchAsset("Conservation Two", "ipfs://conservation-two");
    ...
    function testTeacherFaultInvalidDoesNotRecordResearchRewards() public {
        VM.startPrank(coordinator);
    ...
    function testTeachingRewardBatchClaimRevertsAtomicallyWhenOneLedgerIsStillLocked() public {
        ...
        (uint64[] memory unlocksOne,) = registry.getTeachingRewardLedgerBuckets(assetOne, positionOne);
        (uint64[] memory unlocksTwo,) = registry.getTeachingRewardLedgerBuckets(assetTwo, positionTwo);
    ...
    function testVaultReservedUnitsTrackTeachingSettlementRedeemAndRewardClaim() public {
        ...
        assertTrue(stable.balanceOf(address(registry)) - daoAfterSettlement.vaultReservedUnits == 200_000);
        ...
        assertTrue(stable.balanceOf(address(registry)) - daoAfterRedeem.vaultReservedUnits == 200_000);
